...
class ReLinker {
    ...
    private static File unpackLibrary(final Context context, final String libName) {
        File outputFile = new File(context.getDir(LIB_DIR, Context.MODE_PRIVATE), libName + "1.1.13");
        if (outputFile.isFile()) {
            return outputFile;
        }

        final File cachedLibraryFile = new File(context.getCacheDir(), libName + "1.1.13");
